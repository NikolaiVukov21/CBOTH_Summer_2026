...
if(tolower(basename(Folder_Path))!="new_excels"){
  input_folder<-file.path(Folder_Path,"New_Excels")
} else{
...
##+++++++++++++++++++++++++needed Fix: Need to make 'range' scalable for growing Scenario A Log#+++++++++++++++++++++++++
...
write_csv(df_INVIVO_scenarioB_Master,file.path(analysis_ready_folder,"df_INVIVO_scenarioB_Master.csv"))

df_INVIVO_scenarioB_excluded<-df_INVIVO_scenarioB_Master %>% filter(ID %in% vec_exclude)
write_csv(df_INVIVO_scenarioB_excluded,file.path(analysis_ready_folder,"df_INVIVO_scenarioB_excluded.csv"))

df_INVIVO_scenarioB <- df_INVIVO_scenarioB_Master %>%filter(!(ID %in% vec_exclude))
write_csv(df_INVIVO_scenarioB,file.path(analysis_ready_folder,"df_INVIVO_scenarioB.csv"))
