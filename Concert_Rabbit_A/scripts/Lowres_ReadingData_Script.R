...
process_lowres_rabbitdata<- function (data_file, model){

 #Builds a path to the lowres file
 file.path<-file.path(lowres_folder,paste0(data_file,".xlsx"))
 ...
 #Reads the sheets from that file
  ...
  df_all<-map_dfr(sheets,function(sheet){
    read_excel(file.path,range="A13:BA20",sheet=sheet,trim_ws=T,
...
for (f in all_files){
 if(f==log_filename){
  ...
  df_result<-tryCatch({process_lowres_rabbitdata(data_file=data_file,model=model_name)},
                      error=function(e){
                        return(NULL)
                      })
...
write.csv(df_lowres_scenarioA_Master,file.path(analysis_ready_folder,"df_lowres_scenarioA_Master.csv"),row.names=FALSE,quote=FALSE)

df_lowres_scenarioA_internal<-df_lowres_scenarioA_Master |> filter(ID %in% vec_internal)
write.csv(df_lowres_scenarioA_internal,file.path(analysis_ready_folder,"df_lowres_scenarioA_internal.csv"),row.names=FALSE,quote=FALSE)

df_lowres_ScenarioA<-df_lowres_scenarioA_Master |> filter(ID%in% vec_include)
write.csv(df_lowres_ScenarioA,file.path(analysis_ready_folder,"df_lowres_ScenarioA.csv"),row.names=FALSE,quote=FALSE)
