...
if(requireNamespace("agricolae", quietly = TRUE)==FALSE) install.packages ("agricolae")
if(requireNamespace("readxl", quietly = TRUE)==FALSE) install.packages ("readxl")
if(requireNamespace("openxlsx", quietly = TRUE)==FALSE) install.packages ("openxlsx")
if(requireNamespace("dplyr", quietly = TRUE)==FALSE) install.packages ("dplyr")
if(requireNamespace("tidyr", quietly = TRUE)==FALSE) install.packages ("tidyr")
if(requireNamespace("ggplot2", quietly = TRUE)==FALSE) install.packages ("ggplot2")
if(requireNamespace("tidyverse", quietly = TRUE)==FALSE) install.packages ("tidyverse")
if(requireNamespace("patchwork", quietly = TRUE)==FALSE) install.packages ("patchwork")
if(requireNamespace("ggsignif", quietly = TRUE)==FALSE) install.packages ("ggsignif")
if(requireNamespace("knitr", quietly = TRUE)==FALSE) install.packages ("knitr")
...
GroupStats<-function(data){
  ...
  Modeling_Values<-data %>% 
    ...
    mutate(Hb=coalesce(Hb,EM_Hb),
           #Hb=ifelse(HMT_model==ref_model," ",Hb),
           EMT_model=factor(HMT_model,levels=Defined_Grouping)) %>% 
    mutate(stat_group=case_when(
      !is.na(Alb)& Alb=="A" ~paste(HMT_model,Hb,"A",sep="_"),
      HMT_model==ref_model~paste(HMT_model),
      TRUE~paste(HMT_model,Hb,sep="_")
    ),
    stat_group=factor(stat_group))
    
    Group_Statistics<-list()
    
    for (m in unique(Modeling_Values$stat_group)){
      for (v in variables){
        ...
        Group_Statistics<-append(Group_Statistics,list(
          data.frame(
            Model=m,
            Var=v,
            Mean=mean(values,na.rm=FALSE),
            SD=sd(values,na.rm=FALSE),
            Med=median(values,na.rm=FALSE),
      ...
      Group_Statistics<-do.call(rbind,Group_Statistics)
      return(Group_Statistics)
}
TestStats<-GroupStats(Cleaned_df)
